...
const Skills = () => {
  ...
  const skills = [
    ...
    { name: 'PHP', level: 82, color: 'from-purple-500 to-indigo-500' },
    { name: 'Node.js', level: 78, color: 'from-green-500 to-emerald-500' },
    { name: 'Tailwind CSS', level: 92, color: 'from-cyan-500 to-blue-500' },
    { name: 'Git', level: 86, color: 'from-gray-500 to-gray-700' },
          ...
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 200 200">
            ...
            <defs>
              <linearGradient id={`gradient-${index}`} x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" className={`stop-color-${skill.color.split(' ')[1]}`} />
                <stop offset="100%" className={`stop-color-${skill.color.split(' ')[3]}`} />
              </linearGradient>
            </defs>
          </svg>
        ...
        <ScrollReveal delay={300}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 justify-items-center">
            {skills.map((skill, index) => (
              <div key={skill.name} className={`stagger-${Math.min(index + 1, 6)}`}>
